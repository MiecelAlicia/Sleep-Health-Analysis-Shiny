---
title: "Shiny"
author: "Marcelline Cathrine Wilison"
date: "2024-05-24"
...
df_sleep$Sleep.Disorder <- df_sleep2$Sleep.Disorder %>% 
  factor(., levels = c("Sleep Apnea", "Insomnia", "None"))
...
```{r}
# Convert Sleep.Disorder to a factor with specified levels
df_sleep_clean$Sleep.Disorder <- df_sleep_clean$Sleep.Disorder %>% 
  factor(levels = c("Sleep Apnea", "Insomnia", "None"))
...
vis_1 <- table(df_sleep_clean$Occupation, df_sleep_clean$Sleep.Disorder) %>% 
  ...
  scale_fill_manual(values = c("Sleep Apnea" = "#3C7237", 
                               "Insomnia" = "#5D9D57", 
                               "None" = "#9BDF96"),
                    breaks = c("Sleep Apnea", "Insomnia", "None")) +
  theme_minimal() +
...
```{r}
# Occupation per Group
df_sleep_clean <- df_sleep_clean %>%
  mutate(Occupation_Group = case_when(
    Occupation %in% c("Doctor", "Nurse") ~ "Medical",
    Occupation %in% c("Salesperson", "Manager", "Sales Representative", "Accountant", "Lawyer") ~ "Corporate",
    Occupation %in% c("Software Engineer", "Engineer") ~ "Tech",
    Occupation %in% c("Teacher", "Scientist") ~ "Education"
  ))
```

```{r}
df_sleep_clean$Sleep.Disorder <- factor(df_sleep_clean$Sleep.Disorder, levels = c("Sleep Apnea", "Insomnia", "None"))

# Create the plot
occgroup_sd <- table(df_sleep_clean$Occupation_Group, df_sleep_clean$Sleep.Disorder) %>% 
  as.data.frame(.) %>% 
  group_by(Var1) %>% 
  mutate(Total = sum(Freq),
         Percentage = Freq * 100 / Total) %>% 
  ...
  ggplot(., aes(x = Var1, 
                y = Freq, 
                fill = Var2, 
                text = paste("Occupation Group :", Var1,
                             "<br>Sleep Disorder :", Var2,
                             "<br>Percentage :", round(Percentage, 1), "%"))) +
  geom_bar(position = "fill", 
           stat = "identity") +
  ...
  theme(axis.text.x = element_text(angle = 45, 
                                   hjust = 1)) +
  labs(title = "Percentage of Sleep Disorder per Occupation Group",  
       x = "Occupation Group", 
       y = "Percentage", 
       fill = "Sleep Disorder")

ggplotly(occgroup_sd, tooltip = "text")
...
```{r}
vis_2 <- table(df_sleep_clean$Occupation_Group, df_sleep_clean$BMI.Category) %>% 
  as.data.frame(.) %>% 
  group_by(Var1) %>% 
  mutate(Total = sum(Freq),
         Percentage = Freq * 100 / Total) %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, 
                                   hjust = 1)) +
  labs(title = "Percentage of BMI Category per Occupation Group",  
       x = "Occupation Group", 
       y = "Percentage", 
       fill = "BMI Category")
